#!/usr/bin/env python3
"""
Test Currency Rounding Fix with Mocked Exchange Rates
...
using mocked exchange rates instead of calling the API.
"""
...
logger = logging.getLogger("test_currency_rounding_fix_mock")
...
# Mock function for get_exchange_rate
def mock_get_exchange_rate(from_currency, to_currency, **kwargs):
    """
    Mock function for get_exchange_rate that returns fixed rates
    ...
    Returns:
        float: A fixed exchange rate
    """
    # Define fixed exchange rates for testing
    rates = {
        ("USD", "NTD"): 32.33,
        ("RMB", "NTD"): 4.45,
        ("NTD", "NTD"): 1.0,
    ...
    # Normalize currency codes (remove R- prefix)
    ...
    # Return the rate if it exists, otherwise return 1.0
    return rates.get((from_curr, to_curr), 1.0)

@patch('exchange_rate_query.get_exchange_rate', mock_get_exchange_rate)
...
@patch('exchange_rate_query.get_exchange_rate', mock_get_exchange_rate)
def test_oba_0000027_calculation(fixed_module):
    """
    ...
    logger.info(f"  Sum using Decimal: {float(decimal_total)}")
...
def main():
    ...
    test_oba_0000027_calculation(fixed_module)
