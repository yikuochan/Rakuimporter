...
## How to Verify
You can verify these changes by running the unit tests:

```bash
python -m unittest test_currency_handling.py
```

Or by running the script with the `--dry-run` and `--sample-payload` options on data that includes NTD, JPY, or PHP as non-home currencies:

```bash
python process_japan_exports.py --dry-run --sample-payload=sample_output.json Test-VPA-0000087.json
```

The generated currency modification report will show that these currencies are now being transformed with the "R-" prefix when used as non-home currencies.
