...
The script uses environment variables for secure API configuration. This prevents sensitive credentials from being hard-coded in the source code.
...
```
ERP_CLIENT_ID=your_client_id_here
ERP_CLIENT_SECRET=your_client_secret_here
ERP_TOKEN_URL=https://login.microsoftonline.com/6b83c27c-aa6d-475a-9933-5c34bb008d73/oauth2/v2.0/token
ERP_API_URL=https://api.businesscentral.dynamics.com/v2.0/6b83c27c-aa6d-475a-9933-5c34bb008d73/Staging/ODataV4/Company('VCT')/PurchaseJournals
ERP_SCOPE=https://api.businesscentral.dynamics.com/.default
```

For convenience, you can copy the `.env.example` file and update it with your actual credentials:
...
If environment variables are not set, the script will use the following default values (except for credentials which are required):
